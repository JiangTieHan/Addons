...
local SendWho = C_FriendList.SendWho  --api更新
local min = min
local max = max
...
local InviteUnit = C_PartyInfo.InviteUnit  --api更新
...
--修改图标
addon.ICON_FILE = "Interface\\AddOns\\WhisperPop\\Sounds\\normal-up.tga"  --  "Interface\\Icons\\INV_Letter_05"
...
function addon:GetDisplayName(text, forceRealm)
    if (self:IsBattleTag(text)) then
        local name,id = self:IsBattleTag(text)
        return name
...
function addon:GetBNInfoFromTag(tag)
	...
	for i = 1, count do
		local infoTab = C_BattleNet.GetFriendAccountInfo(i)
		if  infoTab and  infoTab.battleTag == tag then
			local bnName,bnID = addon:IsBattleTag(tag)
			local bnOnLine, characterName 
			bnOnLine, characterName, realmName= infoTab.gameAccountInfo.isOnLine, infoTab.gameAccountInfo.characterName ,infoTab.gameAccountInfo.realmName 
			return bnName,bnID, bnOnLine, characterName, realmName
		end
	end
end
...
function addon:IsBattleTag(name)
    if type(name) == "string" then
        local _, _, bnName, bnID = strfind(name, "(.+)#(%d+)$")
        return bnName, bnID
    end
end
...
function addon:HandleAction(name, action)
    ...
    local bnName,bnID,bnOnLine, characterName, realmName
    if (addon:IsBattleTag(name)) then
        bnName,bnID, bnOnLine, characterName, realmName  = self:GetBNInfoFromTag(name)
        if not bnID then
            return
        end
    end
    ...
    elseif action == "WHO" then  
        SendWho("n-"..(bnName or name))  --api更新
    elseif action == "INVITE" then
        if bnID then
			local realm = ""
			if myRealmName ~= realmName then
				realm = "-"..realmName
			end
			print(characterName..realm)
            InviteUnit(characterName..realm)         --api更新
